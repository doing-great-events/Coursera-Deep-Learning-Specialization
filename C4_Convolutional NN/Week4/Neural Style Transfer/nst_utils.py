...
def load_vgg_model(path):
    ...
    def _weights(layer, expected_layer_name):
        ...
        W = wb[0][0][0][0][0]      # vgg-19
        b = wb[0][0][0][0][1]      # vgg-19
        layer_name = vgg_layers[0][layer][0][0][3]# vgg-19
...
def generate_noise_image(content_image, noise_ratio = CONFIG.NOISE_RATIO):
    ...
    #np.random.seed(19)
    noise_image = np.random.uniform(-10, 10, (1, CONFIG.IMAGE_HEIGHT, CONFIG.IMAGE_WIDTH, CONFIG.COLOR_CHANNELS)).astype('float32')
...
def savep_image(path, image):
    
    # Un-normalize the image so that it looks good
    image = image + CONFIG.MEANS
    
    # Clip and Save the image
    image = np.abs(image[0]).astype('uint8')
    image = np.clip(image[0], 0, 255).astype('uint8')
    #scipy.misc.imsave(path, image)
    imageio.imwrite(path, image)